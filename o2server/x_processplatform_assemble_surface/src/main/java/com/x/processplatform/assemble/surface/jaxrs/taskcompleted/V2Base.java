...
abstract class V2Base extends StandardJaxrsAction {
	...
	protected Predicate toFilterPredicate(EffectivePerson effectivePerson, Business business, FilterWi wi)
		...
		if (ListTools.isNotEmpty(wi.getCreatorPersonList())) {
			...
			p = cb.and(p, root.get(TaskCompletedStatic.creatorPerson).in(person_ids));
		}
		if (ListTools.isNotEmpty(wi.getCreatorUnitList())) {
			List<String> unit_ids = business.organization().unit().list(wi.getCreatorUnitList());
			p = cb.and(p, root.get(TaskCompletedStatic.creatorUnit).in(unit_ids));
		}
		...
		if (completed != notCompleted) {
			if (completed) {
				p = cb.and(p, cb.equal(root.get(TaskCompletedStatic.completed), true));
			} else {
				p = cb.and(p, cb.or(cb.isNull(root.get(TaskCompletedStatic.completed)),
						cb.equal(root.get(TaskCompletedStatic.completed), false)));
			}
		}
		...
		if (StringUtils.isNotEmpty(key)) {
			key = "%" + key + "%";
			p = cb.and(p,
					cb.or(cb.like(root.get(TaskCompletedStatic.title), key), cb.like(root.get(TaskCompletedStatic.serial), key),
							cb.like(root.get(TaskCompletedStatic.creatorPerson), key),
							cb.like(root.get(TaskCompletedStatic.creatorUnit), key)));
		}
		if (StringUtils.isNotEmpty(wi.getTitle())) {
			String title = StringTools.escapeSqlLikeKey(wi.getTitle());
			if (StringUtils.isNotEmpty(title)) {
				p = cb.and(cb.like(root.get(TaskCompletedStatic.title), "%" + title + "%"));
			}
		}
		return p;
	}
