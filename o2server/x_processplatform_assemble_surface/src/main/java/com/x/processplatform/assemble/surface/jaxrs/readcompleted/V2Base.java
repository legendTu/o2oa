...
abstract class V2Base extends StandardJaxrsAction {
	...
	protected Predicate toFilterPredicate(EffectivePerson effectivePerson, Business business, FilterWi wi)
		...
		if (ListTools.isNotEmpty(wi.getCreatorPersonList())) {
			...
			p = cb.and(p, root.get(ReadCompletedStatic.creatorPerson).in(person_ids));
		}
		if (ListTools.isNotEmpty(wi.getCreatorUnitList())) {
			List<String> unit_ids = business.organization().unit().list(wi.getCreatorUnitList());
			p = cb.and(p, root.get(ReadCompletedStatic.creatorUnit).in(unit_ids));
		}
		...
		if (ListTools.isNotEmpty(wi.getCompletedTimeMonthList())) {
			p = cb.and(p, root.get(ReadCompletedStatic.completedTimeMonth).in(wi.getCompletedTimeMonthList()));
		}
		...
		if (completed != notCompleted) {
			if (completed) {
				p = cb.and(p, cb.equal(root.get(ReadCompletedStatic.completed), true));
			} else {
				p = cb.and(p, cb.or(cb.isNull(root.get(ReadCompletedStatic.completed)),
						cb.equal(root.get(ReadCompletedStatic.completed), false)));
			}
		}
		...
		if (StringUtils.isNotEmpty(key)) {
			key = "%" + key + "%";
			p = cb.and(p,
					cb.or(cb.like(root.get(ReadCompletedStatic.title), key), cb.like(root.get(ReadCompletedStatic.serial), key),
