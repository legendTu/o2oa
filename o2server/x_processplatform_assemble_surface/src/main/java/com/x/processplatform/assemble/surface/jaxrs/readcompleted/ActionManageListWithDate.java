...
class ActionManageListWithDate extends BaseAction {
	...
	private List<ReadCompleted> list(Business business, Date startTime, Date endTime) throws Exception {
		...
		if (startTime != null) {
			p = cb.and(p, cb.greaterThanOrEqualTo(root.get(ReadCompletedStatic.createTime), startTime));
		}
		if (endTime != null) {
			p = cb.and(p, cb.lessThan(root.get(ReadCompletedStatic.createTime), endTime));
		}
