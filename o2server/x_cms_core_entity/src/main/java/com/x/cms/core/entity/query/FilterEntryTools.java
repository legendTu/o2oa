			...
			if (filter.getFormatType().equals(FormatType.booleanValue)) {
				if (StringUtils.isNotEmpty(filter.getValue())) {
					if (Comparison.isNotEquals(filter.getComparison())) {
						p = cb.and(p,
								cb.notEqual(root.get(ItemStatic.booleanValue), BooleanUtils.toBoolean(filter.getValue())));
					} else {
						p = cb.and(p,
								cb.equal(root.get(ItemStatic.booleanValue), BooleanUtils.toBoolean(filter.getValue())));
					}
				}
				...
				if (StringUtils.isNotEmpty(filter.getValue())) {
					...
					if (Comparison.isNotEquals(filter.getComparison())) {
						p = cb.and(p, cb.notEqual(root.get(ItemStatic.numberValue), value));
					} else if (Comparison.isGreaterThan(filter.getComparison())) {
						p = cb.and(p,
								cb.greaterThan(root.get(ItemStatic.numberValue), NumberUtils.toDouble(filter.getValue())));
					} else if (Comparison.isGreaterThanOrEqualTo(filter.getComparison())) {
						p = cb.and(p, cb.greaterThanOrEqualTo(root.get(ItemStatic.numberValue), value));
					} else if (Comparison.isLessThan(filter.getComparison())) {
						p = cb.and(p, cb.lessThan(root.get(ItemStatic.numberValue), value));
					} else if (Comparison.isLessThanOrEqualTo(filter.getComparison())) {
						p = cb.and(p, cb.lessThanOrEqualTo(root.get(ItemStatic.numberValue), value));
					} else {
						p = cb.and(p, cb.equal(root.get(ItemStatic.numberValue), value));
					}
				}
				...
				if (StringUtils.isNotEmpty(filter.getValue())) {
					if (DateTools.isDateTime(filter.getValue())) {
						...
						if (Comparison.isNotEquals(filter.getComparison())) {
							p = cb.and(p, cb.notEqual(root.get(ItemStatic.dateTimeValue), value));
						} else if (Comparison.isGreaterThan(filter.getComparison())) {
						...
						if (Comparison.isNotEquals(filter.getComparison())) {
							p = cb.and(p, cb.notEqual(root.get(ItemStatic.dateValue), value));
						} else if (Comparison.isGreaterThan(filter.getComparison())) {
						...
						if (Comparison.isNotEquals(filter.getComparison())) {
							p = cb.and(p, cb.notEqual(root.get(ItemStatic.timeValue), value));
						} else if (Comparison.isGreaterThan(filter.getComparison())) {
				...
				if (Comparison.isNotEquals(filter.getComparison())) {
					p = cb.and(p, cb.notEqual(root.get(ItemStatic.stringShortValue), value));
				} else if (Comparison.isGreaterThan(filter.getComparison())) {
					p = cb.and(p, cb.greaterThan(root.get(ItemStatic.stringShortValue), value));
				} else if (Comparison.isGreaterThanOrEqualTo(filter.getComparison())) {
					p = cb.and(p, cb.greaterThanOrEqualTo(root.get(ItemStatic.stringShortValue), value));
				} else if (Comparison.isLessThan(filter.getComparison())) {
					p = cb.and(p, cb.lessThan(root.get(ItemStatic.stringShortValue), value));
				} else if (Comparison.isLessThanOrEqualTo(filter.getComparison())) {
					p = cb.and(p, cb.lessThanOrEqualTo(root.get(ItemStatic.stringShortValue), value));
				} else if (Comparison.isLike(filter.getComparison())) {
					p = cb.and(p, cb.like(root.get(ItemStatic.stringShortValue), "%" + value + "%"));
				} else if (Comparison.isNotLike(filter.getComparison())) {
					p = cb.and(p, cb.notLike(root.get(ItemStatic.stringShortValue), "%" + value + "%"));
				} else {
					p = cb.and(p, cb.equal(root.get(ItemStatic.stringShortValue), value));
				}
			}
			if (null == predicate) {
				predicate = p;
	...
	public static Predicate toPredicate(CriteriaBuilder cb, Root<Item> root, FilterEntry filter) throws Exception {
		...
		if (filter.getFormatType().equals(FormatType.booleanValue)) {
			if (StringUtils.isNotEmpty(filter.getValue())) {
				if (Comparison.isNotEquals(filter.getComparison())) {
					/** 不等于返回等于值,在外部运算 */
					p = cb.and(p, cb.equal(root.get(ItemStatic.booleanValue), BooleanUtils.toBoolean(filter.getValue())));
				} else {
					p = cb.and(p, cb.equal(root.get(ItemStatic.booleanValue), BooleanUtils.toBoolean(filter.getValue())));
				}
			}
			...
			if (StringUtils.isNotEmpty(filter.getValue())) {
				...
				if (Comparison.isNotEquals(filter.getComparison())) {
					...
					p = cb.and(p, cb.greaterThan(root.get(ItemStatic.numberValue), NumberUtils.toDouble(filter.getValue())));
				} else if (Comparison.isGreaterThanOrEqualTo(filter.getComparison())) {
					p = cb.and(p, cb.greaterThanOrEqualTo(root.get(ItemStatic.numberValue), value));
				} else if (Comparison.isLessThan(filter.getComparison())) {
					p = cb.and(p, cb.lessThan(root.get(ItemStatic.numberValue), value));
